...
// --- LOCAL MODE (ORIGINALE) ---
void run_local(char *start_path) {
    // ... (Codice locale invariato se ti serve, altrimenti lascialo vuoto o copialo dal tuo backup)
    // Per brevità qui metto solo un placeholder, ma nel tuo codice finale dovresti averlo.
    printf("Local mode not fully pasted here to save space (use existing logic).\n");
    exit(0);
}
...
int main() {
    ...
    strcat(PATH, "/bin");
    // Assumi che gli eseguibili siano nella stessa cartella (o aggiungi /bin se serve)
    // strcat(PATH, "/bin"); 

    int choice;
    while(1) {
        printf("\n=== DRONE CONTROL ===\n");
        printf("1. Networked (Assignment 3)\n2. Local (Assignment 2)\n3. Quit\nSelection: ");
        fflush(stdout);
        ...
        if(choice == 1) {
            ...
            run_local(PATH); // Assicurati di avere la funzione implementata o copiata dal vecchio codice
            break;
        } else if (choice == 3) {
            return 0;
        }
